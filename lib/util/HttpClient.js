...
HttpClient.prototype._startRequest = function (request) {
  ...
  try {
      httpRequest = this._request(request);
  } catch (error) {
      return setImmediate(handleRequestError, error);
  }
  ...
  httpRequest.on('response', function (httpResponse) {
    ...
    self._logger.info('Got response', httpResponse.statusCode);
  ...
  function handleRequestError(error) {
    self._logger.info('Got error ', error);
    self._startNextRequest(request);
