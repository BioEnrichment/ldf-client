...
var HttpClient = require('../util/HttpClient'),
    ...
    Logger = require('../util/ExecutionLogger')('FragmentsClient'),
    _ = require('lodash');
...
Fragment.prototype._read = function (count, done) {
  Logger.debug('Attempting to read from Fragment, page is ' + this._page)
  Logger.debug('Fragment is ' + this)
  var item;
...
Fragment.prototype.loadFromUrl = function (pageUrl) {
  ...
  rawPage.on('error', function (error) {
      fragment.setProperty('metadata', { hadError: true })
      fragment.emit('error', error);
  });
  ...
  rawPage.getProperty('statusCode', function (statusCode) {
    ...
    rawPage.getProperty('contentType', function (contentType) {
      ...
      parsedPage.on('error', function (error) {
          fragment.setProperty('metadata', { hadError: true })
          fragment.emit('error', error);
      });
