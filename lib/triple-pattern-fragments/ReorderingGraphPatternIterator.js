...
function ReorderingGraphPatternIterator(parent, pattern, options) {
  ...
  Logger.debug('MAKING REORDERTHINGY parent', parent ? JSON.stringify(parent._pattern) : 'none', 'pattern', JSON.stringify(this._pattern))
}
...
ReorderingGraphPatternIterator.prototype._createTransformer = function (bindings) {
  ...
  var subPatterns = _.sortBy(rdf.findConnectedPatterns(boundPattern), function (patterns) {
    ...
    Logger.debug(NCALL, 'Start:', remainingPatterns + ' pattern(s) left to check for ' + JSON.stringify(subPattern))
  ...
  subPattern.forEach(function (triplePattern, index) {
    Logger.debug(NCALL, 'checking:', JSON.stringify(triplePattern))
    var fragment = this._client.getFragmentByPattern(triplePattern);
    fragment.getProperty('metadata', function (metadata) {
      Logger.debug(NCALL, 'metadata arrived for:', JSON.stringify(triplePattern))
      ...
      if(metadata.hadError === true) {
          Logger.debug(NCALL, '>>>>>>>>>>>>> hadError was true for', JSON.stringify(triplePattern))
          -- remainingPatterns
          return
      }
      ...
      if (!metadata.totalTriples) {
        ...
        Logger.debug(NCALL, 'total triples for', JSON.stringify(triplePattern), 'is', metadata.totalTriples)
      }
      ...
      if (--remainingPatterns === 0) {
          Logger.debug(NCALL, 'i checked', subPattern.length ,'patterns and the best one is', JSON.stringify(subPattern[bestIndex]))
        pipeline.source = createPipeline(subPattern.splice(bestIndex, 1)[0]);
      } else {
        Logger.debug(NCALL, remainingPatterns + ' pattern(s) left to check for ' + JSON.stringify(subPattern))
      }
    });
    // If the fragment errors, set hadError: true in the metadata
    fragment.on('error', function (error) {
      Logger.debug(NCALL, '>>>>>>>>> errored:', JSON.stringify(triplePattern))
