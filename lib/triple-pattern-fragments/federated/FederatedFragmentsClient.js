  ...
  this._clients.forEach(function (client) {
    ...
    if (!empty) {
      var fragment = client.getFragmentByPattern(pattern);
        ...
        //console.log('ffc getFragmentByPattern: get metadata for', JSON.stringify(pattern))
        
      fragment.getProperty('metadata', function (metadata) {
        //console.log('ffc getFragmentByPattern: got metadata for', JSON.stringify(pattern))
        if (metadata.totalTriples === 0) {
            //console.log('pattern', JSON.stringify(pattern), 'is empty')
    ...
    //console.log('ffc getFragmentByPattern: constructing CompoundFragment from ' + fragments.length + ' fragments')
...
function CompoundFragment(fragments, options) {
    ...
    //console.log(ID + ' is ' + fragments.join(','))
  ...
  fragments = this._fragments = _.indexBy(fragments, getIndex);
    ...
    //console.log(ID + ' error threshold starts at ' + errorThreshold)
    this.ID = ID
  ...
  _.each(fragments, function (fragment, index) {
    ...
    var processMetadata = _.once(function (metadata) {
      ...
      if(metadata.hadError) {
          //console.log(ID, 'i got the metadata for ' + fragment + ' and it had hadError true')
        handleError()
        return
      }
    ...
    var fragmentDone = _.once(function () {
      ...
      if (--fragmentsPending === 0) {
          //console.log(ID, 'No more fragments pending in compound fragment')
    ...
    fragment.once('error', function (error) {
      ...
      function handleError(error) {
          if(errored)
              return
          errored = true
        //console.log(ID, 'CompoundFragment handleError: errorThreshold is now ' + (errorThreshold) + ' (we have ' + numFragments + ' fragments)')
        //console.log(ID, 'Error is', error)
          // Only error if the threshold across fragments has been reached
          if (errorThreshold -- === 0) {
            //console.log(ID, 'reached error threshold of ffc CompoundFragment')
...
CompoundFragment.prototype._read = function (count, done) {
  var fragments = this._fragments;
    //console.log(this.ID + ' READ ' + count)
  for (var index in fragments) {
    ...
    // Try to read as much items from the fragment as possible
      ...
      for(;;) {
      item = fragment.read()
        //console.log(this.ID + ' ' + index + ' returned ' + item)
